...
const Blog = () => {
  ...
  return <>
      ...
      <main className="min-h-screen bg-background pt-24">
          ...
          <div className="container mx-auto px-6">
            <div className="max-w-3xl mx-auto text-center">
              ...
              <div className="relative max-w-xl mx-auto">
                ...
                <Input type="text" placeholder="Search articles..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-12 pr-12 py-6 text-lg rounded-full border-border/60 bg-card focus-visible:ring-primary" />
                {searchQuery && <button onClick={clearSearch} className="absolute right-4 top-1/2 -translate-y-1/2 p-1 rounded-full hover:bg-muted transition-colors">
                    <X className="w-5 h-5 text-muted-foreground" />
                  </button>}
              </div>
              
              {searchQuery && <p className="mt-4 text-sm text-muted-foreground">
        ...
        {/* Featured Articles */}
        {showFeatured && <section className="py-12">
            <div className="container mx-auto px-6">
                ...
                {featuredPosts.map(post => <Link key={post.id} to={`/blog/${post.id}`}>
                    <Card className="h-full group hover:shadow-xl transition-all duration-300 border-border/60 bg-card hover:border-primary/30">
                      <CardHeader className="pb-3">
                        <Badge variant="outline" className={`w-fit ${categoryColors[post.category] || "bg-muted"}`}>
                          {post.category}
                        </Badge>
                      </CardHeader>
              ...
              <aside className="lg:w-64 flex-shrink-0">
                <div className="sticky top-28">
                  ...
                  <div className="flex flex-wrap lg:flex-col gap-2">
                    <button onClick={() => setSelectedCategory("All")} className={`flex items-center justify-between px-4 py-2 rounded-lg text-sm transition-all duration-200 ${selectedCategory === "All" ? "bg-primary text-primary-foreground font-medium" : "bg-card hover:bg-accent/50 text-foreground/80"}`}>
                      <span>All</span>
                    ...
                    {categories.map(category => <button key={category} onClick={() => setSelectedCategory(category)} className={`flex items-center justify-between px-4 py-2 rounded-lg text-sm transition-all duration-200 ${selectedCategory === category ? "bg-primary text-primary-foreground font-medium" : "bg-card hover:bg-accent/50 text-foreground/80"}`}>
                        <span>{category}</span>
