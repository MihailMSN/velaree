...
const ValuePillars = () => {
  return <div className="grid md:grid-cols-3 gap-8">
      ...
      return <div key={index} className="flex flex-col items-center text-center space-y-4 p-6 rounded-2xl bg-card/50 border border-border/50 hover:border-accent/50 transition-all duration-300 hover:scale-105 animate-fade-in" style={{
        animationDelay: `${index * 0.1}s`
            ...
            <div className="inline-flex items-center px-3 py-1.5 rounded-full border border-accent/50 bg-primary text-destructive-foreground">
              <span className="text-sm font-semibold text-primary-foreground">{pillar.metric}</span>
            </div>
          </div>;
    })}
    </div>;
